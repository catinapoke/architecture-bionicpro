...
Важные проблем, с которыми столкнулась компания:
...
На схеме не хватает системы SSO, которая будет связана и с интернет магазином и с андройд приложением. Также на схему нужно добавить внешнием IdP для отражения требований. 

Access токены используются для обращения к сервису, refresh к SSO системе для обновления токена.
Принципы локального хранения персональной и медицинской информации не нарушены, т.к. место хранения данных не было изменено и они хранятся локально.

![](task1/containers.png)

В качестве прокси был создан сервис bionicpro-auth, который получает от фронта запрос на авторизацию и возвращает редирект ссылку, по которой уже получает code и code_challenge и получает токен. Далее сохраняет его и отдает сессию в виде хедеров (сессия меняется каждый вызов). Есть поддержка refresh-токена, при истечении срока access и refresh токен автоматически обновляются. Для нового сервиса сделал новый клиент в Keycloak c названием reports-auth. 

Добавлена поддержка LDAP и при аутентификации в токен прокидываются роли из `ldap/config.ldif`. В Keycloak отображается новая User Federation с названием bionicpro-ldap.

Настроен маппер ролей, который читает группы из `ou=Groups,dc=example,dc=com` и маппит их в одноимённые realm-роли Keycloak:

- `jane.smith` / `password` получает роль `user`;
- `john.doe` / `password` получает роль `prothetic_user`;
- `alex.johnson` / `password` получает роль `prothetic_user`.
...
- Поднять ClickHouse (OLAP-хранение данных отчётности)
- Оставить PostgreSQL только для метаданных Airflow
- Поднять Airflow
- Подготовить тестовые данные для отчета (эмуляция сбора данных: датчики и данные клиентов)
- Настроить триггер и процесс ETL
- Создать новый сервис с ручкой /reports, которая будет проверять логин запрашивающего
- Добавить новую кнопку во фронтенд для вызова ручки

Схема будущего решения
![](./task2/containers.png)

В качестве данных для отчета будут использоваться следующие данные:
...
Аггрегированные данные(витрина):
- username
- name
- created_at
- prothesis_ids []string
- signals_count int64

Для витрины данных используется materialized view `aggregated_data_mv` в ClickHouse с целевой табличкой `aggregated_data`, а для изначальных данных - таблицы `clients`, `prostheses`, `telemetry`. Airflow загружает CSV-данные в ClickHouse, после чего витрина наполняется при вставке телеметрии.

Для обработки запросов отчета реализовал сервис bionicpro-reports, который читает подготовленную витрину из ClickHouse. Сервис bionicpro-auth теперь работатет как bff, чтобы не добавлять nginx и его настройки по прокисированию. Так сохраняется обработка сессий и в то же время сохраняется проверка токенов - новый сервис сверяет их по публичным ключам keycloak.
...
Необходимо добавить Nginx и Minio. Все запросы будут идти в nginx, которые будут проксировать вызов в минио и кешировать ответ. 

Конфиг добавил в `task3/nginx.conf`

Далее необходимо исправить контракт /reports и возвращать ссылку на url(маска username/yyyy-mm-dd-hh в UTC), в котором уже будут лежать данные, а в самой ручке добавить в начале проверку по пути файла. Если файла нет - загрузить, иначе просто отдать готовую ссылку. 

Для корректной работы также настроена работа CORS.

Пример доступа к кешированному ответу по url
![](task3/cached_url.png)
...
[Диаграмма контейнеров BionicPRO в модели C4:](BionicPRO_C4_model.drawio.xml)

![[69baab3d3dae90f953a431bc06a50ad7_MD5.png]]
...
2. **Подготовьте инструменты, с которыми будете работать.**
    ...
    - Typescript.
    - Keycloak.
3. **Подготовьте репозиторий, куда вы будете загружать решения заданий.** Для этого склонируйте репозиторий по ссылке [https://github.com/Yandex-Practicum/architecture-bionicpro](https://github.com/Yandex-Practicum/architecture-bionicpro). В коде уже есть изменения, которые вы вносили в уроках.
